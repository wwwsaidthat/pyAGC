...
from pyagc.utils import off_diagonal


class CCASSGWithMRLMethod(CCASSGMethod):
    """CCA-SSG + MRL 融合方法。"""

    def __init__(self, *args, mrl_dims: Sequence[int], mrl_weight: float, **kwargs) -> None:
        super().__init__(*args, **kwargs)
        self.method_name = "ccassg_mrl"
    ...
    def _ccassg_mrl_loss(self, z1_full: Tensor, z2_full: Tensor) -> Tensor:
        ...
        for dim in self.mrl_dims:
            ...
            c1 = z1.T @ z1 / z1.shape[0]
            c2 = z2.T @ z2 / z2.shape[0]
            dec = (off_diagonal(c1).pow(2) + off_diagonal(c2).pow(2)).sum()

            losses.append(inv + self.model.lam * dec)
    ...
    def ssl_train_step_full(self, data: Data, device: torch.device, optimizer: torch.optim.Optimizer) -> float:
        ...
        loss = self._ccassg_mrl_loss(z1, z2)
        loss.backward()
        optimizer.step()
